...
Links inside your own pages are handled for you. Polyglot normally rewrites every in-page link to the active language (`/menu/` becomes `/fr/menu/` on a *french* page); with fallbacks off it leaves a link alone when that language has no version of the target, so `/about/` stays `/about/` and resolves directly instead of bouncing through a redirect. Links to pages that *do* have a *french* version are relativized exactly as before.

With this option enabled, be aware of two things:
- **Host catch-all.** You still need one. Nothing inside your site links to `/fr/about/` any more, but everything *outside* it still does: search engines have those URLs indexed from when you shipped fallback pages, and so do bookmarks and inbound links. Without a catch-all those are hard 404s. On Netlify, add one rule per language, placed last: `/fr/* /:splat 301`. Netlify serves a real file before consulting non-forced redirects, so pages that *do* have a *french* translation keep working — verify this on a deploy preview before relying on it, and check that `localize_redirects: true` (see below) doesn't rewrite these catch-all rules.
